...
with g.as_default():
    ...
    with tf.name_scope('LOSS1'):
        loss1_map = conv3d('loss1_map', conv1_2, [3, 3, 3], CLASSES, padding="SAME")    # 512*512*2

    with tf.name_scope('LOSS2'):
        loss2_map = deconv3d('loss2_map', conv2_2, [3, 3, 3], CLASSES, padding="SAME")  # 256*256*2

    with tf.name_scope('DL2'):
        deconv2 = deconv3d('deconv2', conv5_2, [
            3, 3, 3], root_layers, padding='SAME')  # 512*512*64

        fusing2 = tf.concat([conv1_2, deconv2], -1)  # 融合层2 512*512*128

        conv6_1 = conv3d('conv6_1', fusing2, [
            3, 3, 3], root_layers, in_channel=root_layers * 2, padding="SAME")  # 512*512*64
        conv6_2 = conv3d('conv6_2', conv6_1, [
            3, 3, 3], root_layers, in_channel=root_layers, padding="SAME")  # 512*512*64
    ...
    with tf.Session(graph=g) as sess:
        ...
        while iteration < 10000:
            try:
                ...
                iteration += 1
                print("当前迭代次数：%s"%(str(iteration)))
                ans1, ans2, ans3, ans4 = sess.run(
                    [loss, acc, rates, merged], feed_dict={X: x, Y: y, W: w})
                sess.run(train, feed_dict={X: x, Y: y, W: w})
                summary_writer.add_summary(ans4, iteration)
                print("Iteration:%s,loss:%f,acc:%f,rates:%f"%(str(iteration), ans1, ans2, ans3))
                pic = sess.run(pre_img, feed_dict={X: x, Y: y, W: w})
                print(pic.shape)
                cv2.imwrite('./prediction/pre_' + str(iteration) +  '.jpg',
