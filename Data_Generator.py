...
def writeimg(arr_slice,i):
    filename = os.path.join('E:/liverdata/nii/nrrd3D/CompleteData/', 'liver-orig-nor'+str(i)+'.nrrd')
    print(filename)
...
class Data:
    def __init__(self, path, block_size, stride):
        ...
        raw_data_filename = os.listdir(os.path.join(self.root_path,'orig'))
        label_data_filename = os.listdir(os.path.join(self.root_path,'seg'))
    ...
    def generator(self,raw,label):
        for i in range(len(raw)):
            ...
            print(label)
            print(raw)
            raw_data = read_nrrd(self.root_path + '/orig/' + raw[i])
            label_data = read_nrrd(self.root_path + '/seg/' + label[i])
